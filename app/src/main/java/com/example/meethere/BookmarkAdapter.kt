...
class BookmarkAdapter (val locationList : ArrayList<BookmarkItem>) : RecyclerView.Adapter<BookmarkAdapter.CustomViewHolder>(){
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CustomViewHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.activity_single_item, parent, false)
        return CustomViewHolder(view).apply{
            itemView.setOnClickListener{
                val curPos : Int = adapterPosition
                val tempItem : BookmarkItem = locationList.get(curPos)
                Toast.makeText(parent.context, "${tempItem.location}에 대한 내용을 가져올 예정", Toast.LENGTH_LONG).show()
            }
        }
    }
